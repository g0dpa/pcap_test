...
#define TCP_FIND_THL(XX) ((uint8_t)((((uint8_t*)(&(XX)->DRF))[0] & 0xF0) >> 2))
...
struct tcp_hdr {
	...
	uint32_t seq_number;
	uint32_t ack_number;
	uint16_t DRF; // THL, Reserved, Flags
...
void pk_length(const u_char*, int len);
...
void pk_payload(const uint8_t*, uint32_t);
...
int main(int argc, char* argv[]){
  ...
  while (1) {
    ...
    pk_length(packet, header->caplen);
  }
  return 0;
}

void pk_length(const u_char* packet, int len){
...
void pk_tcp(const struct tcp_hdr *packet_tcp, uint16_t length){
  ...
  printf("Dst Port: %d\n",ntohs(packet_tcp->dest));
	...
	pk_payload(&packet_tcp->payload[hl - 20], len);
}

/* print payload as hexademical */
void pk_payload(const uint8_t *Data, uint32_t len) {
